...
function getShellPath(): string {
  ...
  // NOTE: Do NOT use execSync here to query the user's shell PATH.
  // execSync with stdio:'pipe' corrupts Electron's file descriptor table,
  // causing spawn EBADF errors in subsequent child process creation.
  // Instead, build PATH from known locations + version manager directories.
...
function findClaudeCodeCli(): string | null {
  // NOTE: Do NOT use execSync (e.g. `which claude`, `where claude.cmd`) here.
  // execSync with stdio:'pipe' corrupts Electron's file descriptor table,
  ...
  // macOS / Linux: search common paths
  const commonPaths = [
...
function checkClaudeCodeStatus(forceRefresh = false): typeof cachedClaudeCodeStatus {
  ...
  // Step 2: Mark as available if binary exists
  // NOTE: Do NOT use execSync here (e.g. `claude --version`) — it corrupts
  // Electron's file descriptor table and causes `spawn EBADF` on subsequent
  // process spawns (e.g. when launching the Claude CLI for chat).
...
// Pinned sessions are exempt from the 30-day auto-cleanup and the 100-session cap.
// Unpinned sessions: keep last 100 by updatedAt, drop those older than 30 days.
ipcMain.handle('chatHistory:getSessions', (_, companyId: string) => {
  ...
  history.sessions = history.sessions.filter(s =>
    s.pinned || new Date(s.updatedAt).getTime() > thirtyDaysAgo
  )
...
ipcMain.handle('chatHistory:saveSession', (_, companyId: string, session: ChatSession) => {
  ...
  // Preserve fields from the on-disk session when the caller omits them.
  // - `pinned`: explicit values (true/false) are always honored, used by the toggle-pin action.
  // - `titleAutoGenerated`: a save that omits this flag must not clobber the stored value,
  //   otherwise a manually-renamed title would lose its `false` marker and the next AI
  //   response in that session would regenerate (overwriting the user's rename).
  const merged: ChatSession = (() => {
    if (!existing) return session
    const overrides: Partial<ChatSession> = {}
    if (session.pinned === undefined && existing.pinned) {
      overrides.pinned = existing.pinned
      overrides.pinnedAt = existing.pinnedAt
    }
    if (session.titleAutoGenerated === undefined && existing.titleAutoGenerated !== undefined) {
      overrides.titleAutoGenerated = existing.titleAutoGenerated
    }
    return Object.keys(overrides).length ? { ...session, ...overrides } : session
  })()
  ...
  const pinnedAll = history.sessions.filter(s => s.pinned)
  const unpinnedKeep = history.sessions
    .filter(s => !s.pinned)
    .sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())
    .slice(0, 100)
...
ipcMain.handle('git:sync', async (_, repoPath: string, companyId: string, commitMessage: string) => {
  ...
  try {
    ...
    try {
      ...
      console.warn('Git sync: Sparse checkout reconcile failed:', sparseUpdateError)
    }
...
function readSparseConfig(repoPath: string): SparseConfig {
  try {
    const raw = fs.readFileSync(sparseConfigPath(repoPath), 'utf-8')
    const parsed = JSON.parse(raw)
    return { excluded: Array.isArray(parsed.excluded) ? parsed.excluded.filter((x: unknown) => typeof x === 'string') : [] }
  } catch {
    return { excluded: [] }
  }
}

function writeSparseConfig(repoPath: string, cfg: SparseConfig): void {
  const p = sparseConfigPath(repoPath)
...
// Reconcile the on-disk sparse cone to match the persisted exclusion list.
// excluded empty → full checkout (sparse disabled). Otherwise the cone is
// (remote root folders − excluded). Safe: sparse-checkout never deletes data
// from the index/server, only what is materialised in the worktree.
async function applySparseSelection(git: SimpleGit, repoPath: string, excluded: string[]): Promise<void> {
  const cleanExcluded = [...new Set(excluded.filter(Boolean))]
  writeSparseConfig(repoPath, { excluded: cleanExcluded })

  if (cleanExcluded.length === 0) {
    // Nothing excluded → make sure we are on a full checkout.
    const isSparse = (await git.raw(['config', '--get', 'core.sparseCheckout']).catch(() => '')).trim() === 'true'
    if (isSparse) await git.raw(['sparse-checkout', 'disable'])
    return
  }
  ...
  await git.raw(['sparse-checkout', 'init', '--cone'])
  await git.raw(['sparse-checkout', 'set', ...desired])
}

// Read the persisted exclusion list for a repo.
ipcMain.handle('git:sparseGetExclusions', async (_, repoPath: string) => {
  try {
    const safe = validatePath(repoPath)
    return { success: true, excluded: readSparseConfig(safe).excluded }
  } catch (error) {
    return { success: false, excluded: [], error: error instanceof Error ? error.message : 'Unknown error' }
  }
})

// Set the exclusion list and reconcile the worktree to match.
ipcMain.handle('git:sparseSetExclusions', async (_, repoPath: string, excluded: string[]) => {
  try {
    const safe = validatePath(repoPath)
    const git = createGit(safe)
    await applySparseSelection(git, safe, Array.isArray(excluded) ? excluded : [])
    const enabled = (await git.raw(['config', '--get', 'core.sparseCheckout']).catch(() => '')).trim() === 'true'
    return { success: true, enabled, excluded: readSparseConfig(safe).excluded }
  } catch (error) {
    console.error('Sparse set exclusions error:', error)
    return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
  }
})
